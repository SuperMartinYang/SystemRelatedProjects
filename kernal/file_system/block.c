...
/* return the device size (in block), if fail, return -1 */
...
/* return 0 if f at offset of block_num's info is read into block successfully, else -1 */
...
/* returns 0 if write info in block to f at offset of block_num successfully, else -1 */
...
/* return 0 if read f at offset of inum's info to inode successfully, else -1 */
...
/* return 0 if write info in inode to f at offset of inum successfully, else -1 */
...
int get_block() {
    ...
    for(k = 0; k < 8; k++)           // k locate bit 0 in bitb
