...
void * physical_pageAllocLimit(void * low, void * high){
    ...
    while (low < high){
        ...
        low += SIZE_4KB;        // 4KB as a count
    }
    return NULL;
}

// allocate page with high address
...
// allocate a physical page in either high memory of low memory, we try high first
...
void physical_pageFree(void * physicalAddress){
    // lock
    mutex_lock(&physical_bitmapLock);
    // catch any bad calls so we don't have page fault then mark the address as free
    if (physicalAddress != NULL && physicalAddress < physical_maxAddress)
        // set as free, TO-DO, need tot understand
...
int physical_init(struct MULTIBOOT_INFO * m){
    void * physicalAddress;
    // create the lock
    mutex_init(&physcial_bitmapLock);
    // get the mem size from multiboot_info, then init bitmap
    if ((m->flags & MULTIBOOT_FLAG_MEM) != MULTIBOOT_FLAG_MEM)
        return FAIL;
    // calculate the size of the bitmap so we have 1bit for every 4 KB page in actual physical memory
    ...
    if ((m->flags & MULTIBOOT_FLAG_MMAP) == MULTIBOOT_FLAG_MMAP){
        ...
        // if we don't have a memory map, allocate the system memory we know should be reserved;
    ...
    // bitmap tracked on to the end of the kernel image, this avoid us allocating this memory later on
