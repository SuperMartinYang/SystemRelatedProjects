...
/* check the disk whether existed a file system, if not create one */
void my_mkfs (const char * path) {
	char *tmp = (char *) calloc(BLOCKSIZE, 1);        // allocate in heap instead of stack and initial with 0
	read_block(0, tmp);             // super block
	if ( *(int *) tmp == 2018) {    // file system exists
	...
	sb->magic_number = 2018;       // write the magic number
...
struct inode *find(const char *path){
	...
	if (path[strlen(path) - 1] == '/')     // path is directory not file
		d = 1;
	while (path[i] != '\0'){				// separate path into arg array
		if (j == MAX_LEVEL){
			printf("path error\n");
			return NULL;
		}
	...
	for (i = 0; i <= count; i++){			// go thru arg array to locate the target file
		int flag = 0;
		for (j = 0; j < 10; j++){
